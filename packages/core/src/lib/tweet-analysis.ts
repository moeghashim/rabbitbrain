export interface TweetLearningConcept {
	name: string;
	whyItMattersInTweet: string;
}

export interface TweetLearningAnalysis {
	topic: string;
	summary: string;
	intent: string;
	novelConcepts: TweetLearningConcept[];
}
...
export function parseTweetLearningAnalysisText(rawText: string): TweetLearningAnalysis {
	...
	const novelConcepts = conceptsValue.map((concept, index) => {
		...
		return {
			...
			whyItMattersInTweet: readRequiredString(
				concept.whyItMattersInTweet,
				`novelConcepts[${index}].whyItMattersInTweet`,
			),
		};
	});
