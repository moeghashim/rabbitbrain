...
function dayForConcept(day, conceptName, conceptContext) {
    return {
        day,
...
export function buildFeynmanTrack(prioritizedConcepts) {
    if (prioritizedConcepts.length < 5) {
        throw new Error(`At least 5 prioritized concepts are required to build the track, got ${prioritizedConcepts.length}.`);
    }
    const topFive = prioritizedConcepts.slice(0, 5);
    const conceptDays = topFive.map((item, index) => dayForConcept(index + 1, item.concept.name, item.concept.whyItMattersInTweet));
    ...
    const daySix = {
        ...
        tasks: {
            ...
            explain: "Create a single beginner-friendly explanation that connects all five concepts as one coherent story.",
